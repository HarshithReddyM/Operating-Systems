...
int setpriority(int pid, int priority)
{
 ...
 acquire(&ptable.lock);
  for (proc1 = ptable.proc; proc1 < &ptable.proc[NPROC]; proc1++)
  {
   if (proc1->pid == pid)
   {
     proc1->priority = priority;
     if(priority == 0)
     {
       proc1->procpriority = 1;
     }
     else
     {
       proc1->procpriority = 0;
     }
      break;
   }
 }
...
if(sched_policy == 0)
{
    for(proc1 = ptable.proc; proc1 < &ptable.proc[NPROC]; proc1++)
    {
      ...
      switchkvm();
                                          ...
                                          // It should have changed its proc1->state before coming back.
      proc = 0;
    }
}
else
{
   ...
   for (proc1 = ptable.proc; proc1 < &ptable.proc[NPROC]; proc1++)
   {
      ...
      proc = 0;
      if(proc1->running_tick >=RUNNING_THRESHOLD && proc1->pid != 1 && proc1->pid != 2 && proc1->procpriority != 1)
      {
        proc1->priority = 1;
      }
      struct proc *wp;
      for (wp= ptable.proc; wp< &ptable.proc[NPROC]; wp++)
      {
        ...
        if(wp->waiting_tick >= WAITING_THRESHOLD)
        {
           wp->waiting_tick = 0;
           wp->priority = 0;
           wp->running_tick = 0;
   ...
   if(counter == 0)
   {
     for (proc1 = ptable.proc; proc1 < &ptable.proc[NPROC]; proc1++)
     {
       ...
       for (mp = ptable.proc; mp < &ptable.proc[NPROC]; mp++)
       {
         ...
         if (mp->waiting_tick > maximum)
         {
           maximum = mp->waiting_tick;
           proc1 = mp;
         }
       }
       struct proc *wp;
       for (wp= ptable.proc; wp< &ptable.proc[NPROC]; wp++)
       {
         ...
         if(wp->waiting_tick >= WAITING_THRESHOLD || wp->procpriority == 1 )
         {
           wp->waiting_tick = 0;
           wp->priority = 0;
           wp->running_tick = 0;
